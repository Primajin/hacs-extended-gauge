...
const enLocale: FrontendLocaleData = { language: "en", number_format: "decimal_comma" } as any;
...
describe("formatNumber", () =>
{
  ...
  describe("decimal formatting", () =>
  {
    ...
    {
      expect(formatNumber(3.14159, enLocale, { decimalPlaces: 2 })).toBe("3.14");
    });
    ...
    {
      expect(formatNumber(1.5, enLocale, { decimalPlaces: 1, decimalSeparator: "," })).toBe("1,5");
    });
  });
  ...
  describe("thousands separator", () =>
  {
    ...
    {
      expect(formatNumber(1234567, enLocale, { thousandSeparator: "," })).toBe("1,234,567");
    });

    it("does not insert separator when thousandSeparator is empty string", () =>
    {
      expect(formatNumber(1234567, enLocale, { thousandSeparator: "" })).toBe("1234567");
    });

    it("handles a period as the thousands separator", () =>
    {
      expect(formatNumber(1000, enLocale, { thousandSeparator: "." })).toBe("1.000");
    });

    it("preserves the negative sign before the digits", () =>
    {
      expect(formatNumber(-1234, enLocale, { thousandSeparator: "," })).toBe("-1,234");
    });

    it("handles numbers below 1000 without inserting a separator", () =>
    {
      expect(formatNumber(999, enLocale, { thousandSeparator: "," })).toBe("999");
    });
  });
