...
> **Note:** Older configs created before `display_mode` was introduced may still use the `show_needle` / `show_dial` YAML options. These are deprecated but continue to work as a fallback when `display_mode` is not set. We recommend migrating to `display_mode`.
